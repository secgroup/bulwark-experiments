...
// include session class
  ...
  //check http host
  /* WARNING this part can cause a redirect loop when accessing Tomato's admin panel behind a reverse proxy on a non standard port (like 8080)
     If you want to use a COOKIE_DOMAIN different than '', you should take care of the following :
	- if you are using it behind a reverse proxy, make sure the COOKIE_DOMAIN is set without your custom port
 	- if you are not behind a reverse proxy; you have to set the COOKIE_DOMAIN WITH your custom port, like IP:PORT.
     Another option is to configure your reverse proxy to forward the HTTP_HOST as IP:PORT or HOST:PORT 

    The additional condition HTTP_COOKIE_DOMAIN != '' was not in the original code.
*/

  if ($_SERVER['HTTP_HOST'] != HTTP_COOKIE_DOMAIN &&  HTTP_COOKIE_DOMAIN != '') {  
    if ($_SERVER['HTTPS'] == 'on') {
